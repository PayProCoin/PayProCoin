...
B4ARGS="-datadir=$D4"
$PAYPROCOIND $PAYPROCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$PAYPROCOIND $PAYPROCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} PRO"
...
function StartThree {
  $PAYPROCOIND $PAYPROCOINDARGS $B1ARGS &
  B1PID=$!
  $PAYPROCOIND $PAYPROCOINDARGS $B2ARGS &
  B2PID=$!
  $PAYPROCOIND $PAYPROCOINDARGS $B3ARGS &
  B3PID=$!
}
