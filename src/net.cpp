...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (PayProCoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "PayProCoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. PayProCoin Core is probably already running."), addrBind.ToString());
        else
...
{
    boost::this_thread::interruption_point();
    LogPrint("irc", "IRC waiting %d seconds to reconnect\n", nSeconds);
...
bool GetIPFromIRC(SOCKET hSocket, string strMyName, CNetAddr& ipRet)
{
    ...
    LogPrint("irc", "GetIPFromIRC() got userhost %s\n", strHost.c_str());
...
void ThreadIRCSeed()
{
    ...
    LogPrint("irc", "ThreadIRCSeed trying to connect...\n");
    ...
    while (!fShutdown)
    {
        ...
        if (!ConnectSocket(addrConnect, hSocket))
        {
            LogPrint("irc", "IRC connect failed\n");
        ...
        if (strMyName == "")
            strMyName = "x" + boost::lexical_cast<std::string>(GetRand(1000000000));
        ...
        if (nRet != 1)
        {
            ...
            if (nRet == 2)
            {
                LogPrint("irc", "IRC name already in use\n");
                nNameRetry++;
                Wait(10);
                continue;
            }
        ...
        {
            LogPrint("irc", "GetIPFromIRC() returned %s\n", addrFromIRC.ToString().c_str());
            // Don't use our IP as our nick if we're not listening
        ...
        Send(hSocket, strprintf("JOIN #payprocoin%02d\r", channel_number).c_str());
        Send(hSocket, strprintf("WHO #payprocoin%02d\r", channel_number).c_str());
            ...
            if (boost::algorithm::starts_with(strName, "u"))
            {
                ...
                if (DecodeAddress(strName, addr))
                {
                    ...
                    if (addrman.Add(addr, addrConnect, 51 * 60))
                        LogPrint("irc", "IRC got new address: %s\n", addr.ToString().c_str());
                    nGotIRCAddresses++;
                }
                else
                {
                    LogPrint("irc", "IRC decode failed\n");
                }
            }
        }
        closesocket(hSocket);
...
int main(int argc, char *argv[])
{
    ...
    {
        LogPrint("irc", "Error at WSAStartup()\n");
        return false;
    }
