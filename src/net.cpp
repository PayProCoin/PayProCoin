...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("payprocoin-ext-ip");
...
static const char *strMainNetDNSSeed[][2] = {
    {"158.255.208.56", "158.255.208.56"},
    {"162.252.172.55", "162.252.172.55"},
    {"158.255.214.83", "158.255.214.83"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. PayProCoin is probably already running."), addrBind.ToString().c_str());
        else
