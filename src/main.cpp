...
uint256 hashGenesisBlock("0x00000000c114185cd50cd3ab396c61167a7532ddcc31d24cca843dfd333de65d");
...
const string strMessageMagic = "PayProCoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            // Use an exponentially decaying ~5-minute window:
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY + 7) - GetDepthInMainChain());
}
...
void ThreadScriptCheck() {
    RenameThread("payprocoin-scriptch");
...
bool VerifyDB() {
    ...
    int nCheckDepth = GetArg( "-checkblocks", 80);
    if (nCheckDepth == 0)
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000000c114185cd50cd3ab396c61167a7532ddcc31d24cca843dfd333de65d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1402319820;
        block.nBits    = bnProofOfWorkLimit.GetCompact();;
        block.nNonce   = 2290308334;

        if (fTestNet)
        {
            block.nTime    = 2290308334;
            block.nNonce   = 1234567890;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x1a57ba32850899b6155348087919cd7bed8c79b7b02747df3f2c18eeca1e6b93"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x70, 0x70, 0x72, 0x6f };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PayProCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PayProCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PayProCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("payprocoin-miner");
    ...
    try { loop {
        ...
        printf("Running PayProCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PayProCoinMiner terminated\n");
        throw;
    }
}
